...
| `Home`    | one entity per page, cycling every 4 s: a selectable drawn icon (pool of 10 LVGL-primitive glyphs, hidden-group-per-kind like Weather) + label 24 pt accent + value 48 pt (HA state + its unit) + page dots (centred on the configured page count) + status. Mirrors the Frame's Home mode data, as cycling single cards for the round screen |
...
preset — the unit comes straight from the entity, and the icon is chosen
explicitly from a 10-glyph pool. Same catalog idea as the Frame project's Home
